...
class ChatMessage(BaseModel):
    ...
    content: str = Field(..., description="Message text content", example="What is the return policy duration?")


class ChatRequest(BaseModel):
    """Request payload for POST /chat endpoint."""
    tenant_id: str = Field(..., description="Unique identifier for the tenant", example="tenant_acme")
    query: str = Field(..., description="User query or prompt", example="What is the return policy duration?")
    system_prompt: Optional[str] = Field(
        default=None,
        description="Optional tenant persona or additional instructions appended to base grounding prompt",
        example="Answer in a warm tone and sign off as 'ACME Support Team'.",
    )
